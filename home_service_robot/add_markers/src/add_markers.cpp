...
void odomCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  if (!inTransit && !dropoffComplete)
  {
    ...
    if (fabs(x - odom_pick[0]) <= threshold[0] && fabs(y - odom_pick[1]) <= threshold[0] && fabs(w - odom_pick[2]) <= threshold[1])
    {
      inTransit = true;
  ...
  if (inTransit && !dropoffComplete)
  {
    ...
    if (fabs(x - odom_drop[0]) <= threshold[0] && fabs(y - odom_drop[1]) <= threshold[0] && fabs(w - odom_drop[2]) <= threshold[1])
    {
